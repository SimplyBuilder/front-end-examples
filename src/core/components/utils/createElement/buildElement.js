...
const createShadowElement = (data) => {
    const {state, shadow} = data;
    let shadowRoot;
    if (typeof shadow === "string") return DomStore.getElement(state).attachShadow({
        mode: shadow
    });

    const {mode, styles} = shadow;
    if (mode) {
        shadowRoot = DomStore.getElement(state).attachShadow({mode});
        if (styles) {
 ...
 * @param {string|object} [data.shadow] - The shadow mode ('open' or 'closed') if a shadow DOM is to be attached to the created element.
    ...
    try {
        ...
        if(shadow && childElement.dataset?.state) {
            ...
            if(typeof shadow === "object" || typeof shadow === "string") {
                const shadowRootElement = createShadowElement({
                    state, shadow
                });
 ...
 * @module BuildElement
 */
export const BuildElement = methods;